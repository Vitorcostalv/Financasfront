...
const LoginPage = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      addToast({ title: 'Login realizado', description: 'Bem-vindo de volta!', variant: 'success' });
      navigate('/dashboard');
    },
    onError: (error: any) => {
      ...
      <div className="w-full max-w-md space-y-8 rounded-3xl border border-app-border bg-app-panel p-8 shadow-glow">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Acesse sua conta para acompanhar seus lancamentos.
          </p>
        </div>
        ...
        <p className="text-center text-sm text-slate-400">
          Ainda nao tem conta?{' '}
          <Link className="text-app-accent" to="/cadastro">
            Criar cadastro
          </Link>
        </p>
      </div>
